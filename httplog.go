...
func Handler(logger zerolog.Logger) func(next http.Handler) http.Handler {
	...
	return func(next http.Handler) http.Handler {
		fn := func(w http.ResponseWriter, r *http.Request) {
			...
			defer func() {
				var respBody []byte
				if ww.Status() >= 400 || DefaultOptions.Body {
...
func (l *requestLogger) NewLogEntry(r *http.Request) middleware.LogEntry {
	...
	entry.Logger = l.Logger.With().Fields(requestLogFields(r, DefaultOptions.Concise, DefaultOptions.Body)).Logger()
	if !DefaultOptions.Concise {
		entry.Logger.Info().Fields(requestLogFields(r, DefaultOptions.Concise, DefaultOptions.Body)).Msgf(msg)
	}
	return entry
}
